...
import mario_emu.picture_processing
...
# Program parameters
image_processor = mario_emu.picture_processing.PictureAnalyser()
...
def learn(mpi_comm=None, debug=False, render=False, save_name=None, host_name=None):
    ...
    while True:
        ...
        Emulator.eval_population(pop, image_processor, render=render, debug=debug)
        pop.keep_bests()
...
def profile_run():
    for i in range(2):
        ...
        Emulator.eval_population(pop, image_processor)
        pop.next_gen()
    ...
    for (i, gen) in enumerate(pop.list_genomes):
        ...
        with open(infile + ".gv", "w") as f:
            ...
            for j in range(gen.conf.inp, gen.conf.inp + gen.conf.node):
                if gen.used_node[j]:
                    f.write('    {} [label="{}"];\n'.format(j, "{}: {}".format(j, gen.genotype[
                        j - gen.conf.inp].fn_name())))
                    for p in gen.genotype[j - gen.conf.inp].preds():
                        f.write('    {} -> {};\n'.format(p, j))
            for j in range(gen.conf.inp + gen.conf.node, gen.conf.inp + gen.conf.node + gen.conf.out):
                f.write('    {} [label="{}"];\n'.format(j, "output {}: {}".format(j - gen.conf.node - gen.conf.inp,
                                                                                  gen.genotype[
                                                                                      j - gen.conf.inp].fn_name())))
                f.write('    {} -> {};\n'.format(j, "Decision"))
                for p in gen.genotype[j - gen.conf.inp].preds():
...
def main():
    ...
    parser.add_argument(
        "-c",
        "--collector",
        help="Use a remote server to share results between workers.")
    parser.add_argument(
        "-m",
        "--mpi",
